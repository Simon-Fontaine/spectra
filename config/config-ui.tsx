import { APP_CONFIG_PUBLIC } from "@/config/config.public";
import type {
  AppNavigationItem,
  AppSocialItem,
  DashboardNavigationGroup,
  DashboardNavigationItem,
} from "@/types/config";
import {
  CalendarSyncIcon,
  HelpCircleIcon,
  LockIcon,
  Settings2Icon,
  ShieldIcon,
  Users2Icon,
  UsersIcon,
} from "lucide-react";
import Image, { type ImageProps } from "next/image";

export const APP_LOGO = ({ ...props }: Omit<ImageProps, "src" | "alt">) => (
  <Image
    src="/images/icon.png"
    alt={`${APP_CONFIG_PUBLIC.APP_NAME} Logo`}
    {...props}
  />
);

export const ICON_TWITTER = (props: React.SVGProps<SVGSVGElement>) => (
  <svg
    role="img"
    viewBox="0 0 24 24"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <title>X</title>
    <path
      fill="currentColor"
      d="M18.901 1.153h3.68l-8.04 9.19L24 22.846h-7.406l-5.8-7.584-6.638 7.584H.474l8.6-9.83L0 1.154h7.594l5.243 6.932ZM17.61 20.644h2.039L6.486 3.24H4.298Z"
    />
  </svg>
);

export const ICON_YOUTUBE = (props: React.SVGProps<SVGSVGElement>) => (
  <svg
    role="img"
    viewBox="0 0 24 24"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <title>YouTube</title>
    <path
      fill="currentColor"
      d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"
    />
  </svg>
);

export const ICON_GITHUB = (props: React.SVGProps<SVGSVGElement>) => (
  <svg
    role="img"
    viewBox="0 0 24 24"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <title>GitHub</title>
    <path
      fill="currentColor"
      d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"
    />
  </svg>
);

export const APP_SOCIALS = [
  {
    name: "Twitter",
    url: "https://x.com/SpectraOW2",
    icon: ICON_TWITTER,
    disabled: false,
  },
] as AppSocialItem[];

export const APP_NAVIGATION = [
  {
    title: "Roster",
    href: "/roster",
    disabled: false,
    external: false,
  },
  {
    title: "Calendar",
    href: "/calendar",
    disabled: false,
    external: false,
  },
  {
    title: "News",
    href: "/news",
    disabled: false,
    external: false,
  },
  {
    title: "Roster Reveal",
    href: "https://www.youtube.com/watch?v=Ji4bltE3nus",
    disabled: false,
    external: true,
  },
] as AppNavigationItem[];

export const DASHBOARD_MAIN_NAVIGATION = [
  {
    title: "Team",
    url: "/dashboard/team",
    icon: Users2Icon,
    isActive: true,
    items: [
      {
        title: "Roster",
        url: "/dashboard/team/roster",
      },
      {
        title: "Stats",
        url: "/dashboard/team/stats",
      },
      {
        title: "Replays",
        url: "/dashboard/team/replays",
      },
    ],
  },
  {
    title: "Schedule",
    url: "/dashboard/schedule",
    icon: CalendarSyncIcon,
    items: [
      {
        title: "Calendar",
        url: "/dashboard/schedule/calendar",
      },
      {
        title: "Settings",
        url: "/dashboard/schedule/settings",
      },
    ],
  },
  {
    title: "Settings",
    url: "/dashboard/settings",
    icon: Settings2Icon,
    items: [
      {
        title: "General",
        url: "/dashboard/settings/general",
      },
      {
        title: "Profile",
        url: "/dashboard/settings/profile",
      },
      {
        title: "Notifications",
        url: "/dashboard/settings/notifications",
      },
    ],
  },
] as DashboardNavigationGroup[];

export const DASHBOARD_ADMIN_NAVIGATION = [
  {
    title: "User Management",
    url: "/dashboard/admin/user-management",
    icon: UsersIcon,
  },
  {
    title: "Team Management",
    url: "/dashboard/admin/team-management",
    icon: ShieldIcon,
  },
  {
    title: "Access Control",
    url: "/dashboard/admin/access-control",
    icon: LockIcon,
  },
] as DashboardNavigationItem[];

export const DASHBOARD_SECONDARY_NAVIGATION = [
  {
    title: "Help",
    url: `mailto:${APP_CONFIG_PUBLIC.APP_HELP_EMAIL}`,
    icon: HelpCircleIcon,
  },
] as DashboardNavigationItem[];
